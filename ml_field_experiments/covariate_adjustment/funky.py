...
from ml_field_experiments.covariate_adjustment.methods.regression import DIM, Regression
from ml_field_experiments.covariate_adjustment.methods.lin import LinRegression
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="sparsity of response surface polynomials",
    )
    parser.add_argument(
        "--num-reps",
        type=int,
...
def main():
    ...
    output_dir = (
        Path(args.output_dir) / "ATE" / "SinInteraction" / f"{args.method}" / exp_str
    )
