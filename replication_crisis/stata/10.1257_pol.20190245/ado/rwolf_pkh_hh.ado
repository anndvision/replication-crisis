...
//Updated 17 September 2018 by Aaron Berman for use in PKH Long Term Evaluation
//Specifically for adjustment of household outcomes, which drop >99th percentile observations for each outcome separately

cap program drop rwolf_pkh_hh //AB version of ado
program rwolf_pkh_hh, eclass
vers 11.0
#delimit ;
...
[
 ...
 indepexog
 bl(name)
 agebinvars(varlist) //AB added to identify variables for which age-bin controls should be included 
 expvars(varlist) //AB added to identify variables for which 99th percentile and above should be dropped
 *
 ]
;
#delimit cr
...
foreach var of varlist `varlist' {
    ...
    //AB added to incorporate age-bin controls for selected variables 
    if length(`"`agebinvars'"')!=0 {
        local indicator = 0
        foreach x of varlist `agebinvars' {
            if "`var'" == "`x'" local indicator = 1
        }
        if `indicator' == 1 local Xv `controls' agebin*
    }
    //
    if length(`"`indepexog'"')==0 {
        //AB added to incorporate excluding 99th percentile from household regressions
        if length(`"`expvars'"')==0 {
            `q' `method' `var' `ivr1'`indepvar' `otherendog'`ivr2' `Xv' `if' `in' `wt', `options'
        }
        else {
            local indicator_hh = 0
            foreach x of varlist `expvars' {
                if "`var'" == "`x'" local indicator_hh = 1
            }
            if `indicator_hh' == 1 {
                qui summ `var', detail
                `q' `method' `var' `ivr1'`indepvar' `otherendog'`ivr2' `Xv' if `var' < `r(p99)' `in' `wt', `options'
            }
            else {
                `q' `method' `var' `ivr1'`indepvar' `otherendog'`ivr2' `Xv' `if' `in' `wt', `options'
            }
    ...
    foreach var of varlist `varlist' {
        ...
        if length(`"`indepexog'"')==0 {
            ...
            if length(`"`expvars'"')==0 {
                ...
                if `indicator_hh' == 1 {
                    qui summ `var', detail
                    qui `method' `var' `ivr1'`indepvar' `otherendog'`ivr2' `Xv' if `var' < `r(p99)' `in' `wt', `options'
                }
                else {
                    qui `method' `var' `ivr1'`indepvar' `otherendog'`ivr2' `Xv' `if' `in' `wt', `options'
                }
